/*
  clloader receive files with x/y/zmodem
  ...
  **** Modifed for Clanton Ardunio loading.
...
#ifndef CLANTON_LOADER
#define CLANTON_LOADER
#endif

#ifdef CLANTON_LOADER
#include "clloader.h"
static struct daemon_state mystate;
#endif
...
int allow_remote_commands=TRUE;
int junk_path=FALSE;
...
#ifdef CLANTON_LOADER
static int clantonLoaderCmdDetected=0;
char *clantonLoaderCmdStr=0;
int clantonLoaderExecCommand(char *);
int clantonLeavePassThroughState=0;
#endif
...
int my_fclose(FILE *fp)
{
  ...
  if (fp) {
  	ret = fclose(fp);
	/* on line errors, attempt to close fout twice results in segfault. */
	if ( fp == fout)
	    fout=NULL;
  }
  else
  {
     if (Verbose)
	fprintf(stderr,"\nClloader: fclose attempt on closed file\n");
  }
...
bibi(int n)
{
	...
	if ( mystate.slave_pid) {
	     int wait_status;
		if( kill(mystate.slave_pid,SIGKILL )){
			fprintf(stderr,"Attempt to kill child process:pid %d failed\n",mystate.slave_pid);
			perror("Child kill fail");
		}
		waitpid(mystate.slave_pid, &wait_status, WNOHANG); /* get rid of zombe from process table */
	}

	error(128+n,0,_("caught signal %d; exiting"), n);
}

/* called by signal hup  to trigger state change of bootloader */
RETSIGTYPE
hup_signal(int n)
{
   if ( Verbose )
		fprintf(stderr,"signal- hup  signal rxed.. go back to wait for command ... \n",mystate.slave_pid);
   clantonLeavePassThroughState++;
}

RETSIGTYPE
chld_signal_ignore(int n)
{
   if ( Verbose )
		fprintf(stderr,"signal- child ignore  signal rxed\n");
  
}
/* called by signal sketch process died  to trigger state change of bootloader */
RETSIGTYPE
chld_signal(int n)
{
	if ( mystate.slave_pid != 0 )
	{
	    if ( Verbose )
		fprintf(stderr,"sigchld signal rxed.. child pid:%d terminated, go back to wait for command ... \n",mystate.slave_pid);
	    clantonLeavePassThroughState++;
	    mystate.slave_pid = 0 ; // Since the child termianted, no need to kill todo: get info in signal to make sure it was the sketch
	}
	else
	{
	    if ( Verbose )
		fprintf(stderr,"sigchld signal rxed.. terminated. Do not nothing pid:%d \n",mystate.slave_pid);
	}
...
#  endif
	...
	signal(SIGCHLD, chld_signal_ignore);
...
#else
	if (clantonLoaderFSM(npats, patts)==ERROR) {
		exitcode=0200;
		canit(STDOUT_FILENO);
	}
...
/* This code changes funcion depending on exe name!!!  */
	...
	if (Verbose) {
		fprintf(stderr,"cloadler: Chkinvok: %s\n",s);
		fprintf(stderr,"clloader:Force protocol Mode to ZMODEM\n");
	}
...
sys2(const char *s)
{
	...
	if (*s == CL_LOADER_CMD_STARTCHAR )
	{
		if ( clantonLoaderCmdStr = malloc(strlen(s)+1)) {
			strcpy(clantonLoaderCmdStr,s);
			clantonLoaderCmdDetected=1;
			if ( Verbose )
			{
				fprintf(stderr,"Clanton Loader Command Recieved:%s\n",clantonLoaderCmdStr);
			}
		}
		else
		{
			fprintf(stderr,"malloc failed for Loader command \n");
		}
	}
	else {
#endif
...
clantonLoaderExecClear()
{
	clantonLoaderCmdDetected = 0;
	if (clantonLoaderCmdStr) {
		free(clantonLoaderCmdStr);
		clantonLoaderCmdStr = 0;
	}

}
int clantonLoaderExecCommand(char * cmdStr)
{
   int retCode;
   char tmpStr[512];
#if 0
   if ( clantonLoaderCmdDetected) {
	   fprintf(stderr,"Clanton Loader Detected Command:%s\n",cmdStr);

	   if (!strcmp(CL_LOADER_CMD_START_SKETCH_APP,cmdStr) ) {
		   /* Exec the targeted funcion */
		   cmdStr += strlen(CL_LOADER_CMD_START_SKETCH_APP);
		   fprintf(stderr,"Clanton Loader Sub Command:%s\n",cmdStr);

		   retCode = access(CLANTON_SKETCHPROGRAM, (R_OK | X_OK));
		    if ( retCode == ENOENT)  {
			   fprintf(stderr,"Clanton Loader command failed: no file %s\n",CLANTON_SKETCHPROGRAM);
			   return -1;
		   }
		   else if (retCode == EACCES) {
			  /* Try chmod to fix permissions, todo: change to c later. */

			   sprintf(tmpStr,"chmod %s",CLANTON_SKETCHPROGRAM);
			   if ( system(tmpStr) != 0 ) {
				fprintf(stderr,"Clanton Loader command failed:%s\n",tmpStr);
				return -1;
			   }
		   }
		   else if ( retCode == 0) {
			sprintf(tmpStr,"%s %s",CLANTON_SKETCHPROGRAM,cmdStr);
...
int clantonLoaderFSM(int argc, char * argv[])
{
	char * slave_args[MAX_ARGS];
	char * slavename;
	char rbuf_from_sketch[BUFFER_SIZE];
	...
	while (1){
		...
		case FSM_STATE_PASSTHROUGH_INIT:

			/* Check if the sketch program exists */
			if (  access(CLANTON_SKETCHPROGRAM, (R_OK | X_OK)) != 0)
			{
				fprintf(stderr, "%s file does not exist or invalid permissions\n",CLANTON_SKETCHPROGRAM);
				mystate.fsm_state = FSM_STATE_ZMODEM;
				break;
			}
			else {

				/* execv() - a sketch */
				mystate.tty_slave = open("/dev/ptmx", O_RDWR);
				if(mystate.tty_slave < 0){
					fprintf(stderr, "unable to open /dev/ptmx\n");
					return -1;
				}
				grantpt(mystate.tty_slave);
				unlockpt(mystate.tty_slave);
				slavename = ptsname(mystate.tty_slave);

				signal(SIGCHLD, chld_signal);

				/* Fork */
				mystate.slave_pid = fork();
				if (mystate.slave_pid == 0){

					/* Child do execvp() */
					slave_args[0] = CLANTON_LDSO;    /* dynamic uclibc loader */
					slave_args[1] = CLANTON_SKETCHPROGRAM;    /* Name of process */
					slave_args[2] = slavename;		  /* Serial.write => ttyGS0 */
					slave_args[3] = NULL;
					...
					/* Parent manage /tty interface */
					mystate.fsm_state = FSM_STATE_PASSTHROUGH_RUNNING;
					if ( Verbose )
						fprintf(stderr, "Sketch child process started, pid=0%d\n",mystate.slave_pid);
					}
			}
			/* Cleanup dynamic parameter given from host PC side - currently not being passed to sketch */
			if (mystate.sketch_args != NULL)
				free(mystate.sketch_args);

			break;
		case FSM_STATE_PASSTHROUGH_RUNNING:
			/* zero */
			FD_ZERO(&fdrset);  //todo: move td zero/set to PASSTHROUGH_INIT
			...
			default:
				/* Process data */
				if(FD_ISSET(mystate.tty_slave, &fdrset)){

					memset(rbuf_from_sketch, 0x00, sizeof(rbuf_from_sketch));
					ret = read(mystate.tty_slave, rbuf_from_sketch, sizeof(rbuf_from_sketch));
					if(ret < 0){
						...
						ret = write(mystate.tty_to_host,rbuf_from_sketch, ret); //todo check write status
						if((ret =! EAGAIN) && (ret != EWOULDBLOCK)){
							/* Critical - exit out */
							(void)err;
						}
			...
			if ( clantonLeavePassThroughState ){
				int wait_status;
				/* HUP recieved, jump to command loader zmodem state */
				signal(SIGCHLD, chld_signal_ignore);
				mystate.fsm_state = FSM_STATE_ZMODEM;
				...
				if ( mystate.slave_pid) {
					...
					// todo: move to child signal handler to actaully change state, for now just get rid of zombee in process table
					while(1) {
						int wait_pid;
						wait_pid = waitpid(-1, &wait_status, WNOHANG);
						if ( wait_pid == 0 ) { /* No terminated children */
						    if ( Verbose )
							fprintf(stderr,"Waiting for terminated child process:pid %d \n",mystate.slave_pid);
						    sleep(1);
						}
						else
					   	    break;
				...
				// close the pts tty: todo: just reuse if open. 
				close(mystate.tty_slave);

			}
 			break;
		case FSM_STATE_ZMODEM:

			mystate.slave_pid = 0; // we should not ahve any children sketch programs running
			/* Clear our command info */
			clantonLoaderExecClear();

			rxErrorCode = wcreceive(argc, argv);
			if (rxErrorCode == ERROR)
			{
				if (Verbose) {
				   fprintf(stderr, "Errors/timeout waiting to receive commands/data - terminating\n\r");
				}
				// restarting wcreceive in this cases can cause segfaults,
				// underlying use case not supported.
				// loader should be called/restarted by shell if terminates
				canit(STDOUT_FILENO);
				io_mode(0,0);
				exit(0200);
				// todo: restart the process automaitcally here. OR fix wcreceive error cleanup.


			} else {
				if (clantonLoaderCmdDetected ) {
					/* is it a start sketch command */
					if (strncmp(CL_LOADER_CMD_START_SKETCH_APP,
						    clantonLoaderCmdStr,
						    strlen(CL_LOADER_CMD_START_SKETCH_APP))==0 ) {
						/* Get the arguments */

						if (mystate.sketch_args !=0) {
							free(mystate.sketch_args);
							mystate.sketch_args = 0;
						}

						mystate.sketch_args = malloc(strlen(clantonLoaderCmdStr)-strlen(CL_LOADER_CMD_START_SKETCH_APP)+1);
						if (!mystate.sketch_args ){
							fprintf(stderr, "Malloc failed sketch args\n");
							exit(1);
						}

						strcpy(mystate.sketch_args,clantonLoaderCmdStr+strlen(CL_LOADER_CMD_START_SKETCH_APP));
						fprintf(stderr,"Clanton Loader Sub Command:%s\n",clantonLoaderCmdStr);
						clantonLoaderExecClear();
						mystate.fsm_state = FSM_STATE_PASSTHROUGH_INIT;
		   			}
				...
				/*  Default behaviour - Try to start sketch program */
				io_mode(0,1);
				mystate.fsm_state = FSM_STATE_PASSTHROUGH_INIT;
				if ( Verbose) {
					fprintf(stderr,"Zmodem wait finished .. restart to look for sketch\n");
				}
			}
			break;
		}
